...
# 拱火/引战模式
FLAME_WAR_PATTERNS = [
    "你们打一架", "打一架", "打一架看看", "吵起来", "怼他",
    "我站", "站队", "支持", "反对",
    "另一个不行", "那个不行", "这不行", "不行，", "那个垃圾",
    "来来来", "继续", "继续吵", "你们谁对谁错",
    "看戏", "围观", "吃瓜",
    "拱火", "引战", "挑起",
]
    ...
    for pattern in FLAME_WAR_PATTERNS:
        ...
        if pattern.lower() in text_lower:
            ...
            if is_group:
                ...
                # 私聊中相对宽松，但"打一架"等仍不合适
                if pattern in ["打一架", "你们打一架", "打一架看看", "继续吵", "拱火", "引战"]:
...
def rewrite_unsafe_response(
    ...
    *,
    ...
    # 根据风险类型选择改写策略
    if "attack" in safety_result.risk_categories or "insult" in safety_result.risk_categories:
        # 攻击性内容：完全去除攻击词汇
...
def determine_banter_level(
    *,
    ...
    for boundary in existing_boundaries:
        ...
        if any(kw in boundary_lower for kw in ["别阴阳怪气", "别调侃", "认真点", "别攻击"]):
            return "none"
