...
from yqy_bot.core.models import GateDecision, GeneratedResponse, IntentDecision, ParsedMessage
...
class ChatPipeline:
    ...
    def from_config(cls, config: ProjectConfig) -> "ChatPipeline":
        ...
        context_builder = ContextBuilder(config=config, repos=repos, napcat_tools=napcat_tools)
    ...
    async def process_message(
        ...
        *,
        ...
        parsed = payload if isinstance(payload, ParsedMessage) else parse_message_input(payload)
        LOGGER.info(
        ...
        if self._is_ignored(parsed):
            LOGGER.info("[管道] session_id=%s 忽略消息 reason=ignored_or_command", parsed.session_id)
        ...
        gate = self._reply_gate(parsed, group_heat_state)
        LOGGER.info(
            "[管道] session_id=%s 门控 gate_allow=%s group_mode=%s group_heat=%s is_triggered=%s reason=%s is_superuser=%s",
            parsed.session_id,
            gate.allow,
            gate.group_mode,
            group_heat_state,
        ...
        # superusers 跳过冷却限制
        if not self._is_superuser(parsed) and not self.bundle.send_cooldown.can_reply(parsed, now=now):
            gate.allow = False
        ...
        LOGGER.info("[管道] session_id=%s 开始生成回复 emoji_request=%s", parsed.session_id, self.bundle.emoji_service.is_explicit_request(parsed.text))
        if self.bundle.emoji_service.is_explicit_request(parsed.text):
            LOGGER.info("[管道] session_id=%s 表情请求处理", parsed.session_id)
            response = await self.bundle.emoji_service.choose_for_reply(intent, parsed.text)
            if response is not None:
                ...
                context = await self.bundle.context_builder.build(parsed, intent, group_heat_state=group_heat_state)
                response = await self.bundle.safety_guard.check(parsed, response, context)
                response.reply_to_message_id = response.reply_to_message_id or parsed.reply_message_id
                delay_seconds = await self.bundle.human_behavior.typing_delay(parsed, response.text)
                if delay_seconds > 0:
                ...
                self.bundle.send_cooldown.mark_replied(parsed, now=time.time(), group_heat_state=group_heat_state)
                LOGGER.info(
            ...
            LOGGER.info("[管道] session_id=%s 表情请求无响应转入正常流程", parsed.session_id)
        ...
        response = await self.bundle.safety_guard.check(parsed, response, context)
        response = await self.bundle.emoji_service.resolve_response(parsed.text, intent, response)
        response.reply_to_message_id = response.reply_to_message_id or parsed.reply_message_id
        delay_seconds = await self.bundle.human_behavior.typing_delay(parsed, response.text)
        LOGGER.info("[管道] session_id=%s 打字延迟 delay=%.2fs", parsed.session_id, delay_seconds)
        if delay_seconds > 0:
            await asyncio.sleep(delay_seconds)
        if not self.bundle.send_cooldown.can_reply(parsed, now=time.time()):
            LOGGER.info("[管道] session_id=%s 冷却阻塞(发送前) elapsed=%.2fs", parsed.session_id, time.time() - start_time)
            await self.bundle.background.enqueue(parsed, gate, intent, response, context)
            return PipelineResult(
                parsed=parsed,
                gate=gate,
    ...
    def _is_ignored(self, parsed: ParsedMessage) -> bool:
        ...
        if parsed.user_id in set(self.bundle.config.bot.blocked_user_ids):
            return True
        if parsed.is_group and parsed.scope_id in set(self.bundle.config.bot.blocked_group_ids):
            return True
        return False
    ...
    def _reply_gate(self, parsed: ParsedMessage, group_heat_state: str = "quiet") -> GateDecision:
        """判断消息是否应触发回复门控。
        ...
        if parsed.is_private:
            allow = self._private_allow(parsed, explicit_trigger, explicit_emoji_request)
            return GateDecision(
                allow=allow,
                group_mode="private",
                reason="private",
                ...
                is_cooldown_blocked=not allow and not explicit_trigger,
            )
        # 群聊：根据热度状态分层处理
    ...
    def _is_strong_question(self, text: str) -> bool:
        ...
        text = text.strip()
        # 必须有问号
        has_question_mark = any(token in text for token in self.bundle.config.safety.question_mark_tokens)
        if not has_question_mark:
            return False
        # 同时有疑问词才算强问题
        question_words = ("谁", "什么", "怎么", "为什么", "哪", "几", "多少", "干嘛", "如何", "是不是", "能不能", "可以吗")
    ...
    def _has_question(self, text: str) -> bool:
        ...
        text = text.strip()
        # 包含问号
        if any(token in text for token in self.bundle.config.safety.question_mark_tokens):
            return True
        # 包含疑问词
        question_words = ("谁", "什么", "怎么", "为什么", "哪", "几", "多少", "干嘛", "如何", "是不是", "能不能", "可以吗")
        return any(word in text for word in question_words)
    ...
    def _private_allow(
        ...
        if any(token in text for token in self.bundle.config.safety.question_mark_tokens):
            return True
        if any(token in text for token in ("谁", "什么", "怎么", "为什么", "哪", "几", "多少", "干嘛")):
            return True
    ...
    def _has_trigger(self, parsed: ParsedMessage, text: str) -> bool:
        ...
        Returns:
            如果满足触发条件则返回 True
        """
        keywords = self.bundle.config.bot.group_trigger_keywords + self.bundle.config.bot.private_trigger_keywords
        lowered = text.lower()
        if parsed.is_at_bot or parsed.reply_message_id:
            return True
        return any(keyword.lower() in lowered for keyword in keywords)

    def _is_reply_worthy(self, parsed: ParsedMessage, text: str, group_mode: str) -> bool:
        """判断消息是否值得回复。
        ...
        """
        if self._is_low_info(text):
            return parsed.is_private and self.bundle.config.bot.allow_private_short_reply
        # 包含问号的消息值得回复
        if any(token in text for token in self.bundle.config.safety.question_mark_tokens):
            return True
        # 包含疑问词的消息值得回复
        if any(token in text for token in ("谁", "什么", "怎么", "为什么", "哪", "几", "多少", "干嘛", "如何")):
            return True
    ...
    def _is_low_info(self, text: str) -> bool:
        ...
        return len(text.strip()) <= self.bundle.config.safety.low_info_max_chars or any(
            keyword in text for keyword in self.bundle.config.safety.low_info_skip_keywords
        )
